...
export async function getStaticProps({ params }) {
  const md = await getRawFileFromRepo(`canary/examples/${params.slug}/README.md`, '');
  const { content } = matter(md);
  const html = await markdownToHtml(params.slug, '', content);
  return { props: { slug: params.slug, data: examplesData[params.slug], html } };
}
